...
best_param = {"order": (1, 1, 1), "seasonal_order": (0, 0, 0, 0), "trend": "c"}
...
def test_transform(mock_arima, forecast_instance):
    ...
    mock_fit_instance.forecast.return_value = pd.Series(
        [500, 600, 700, 800],
        index=pd.period_range(start=df.index[-1] + 1, periods=4, freq="Q"),
    )
