...
def load_config(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"Error: An error occurred while parsing the YAML file at path {path}: {exc}"
        )
...
def log_results(
    file_path: str = load_config(os.path.join("config", "config.yaml"))["logs"]["path"]
) -> Callable:
    """
...
def log_execution_time(method):
    ...
    def timed(*args, **kwargs):
        ...
        logger.info(
            f"Executing {method.__qualname__} took {execution_time:.4f} seconds"
        )
        return result
