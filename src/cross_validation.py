...
class TimeSeriesCrossValidator(BaseOperation):
    ...
    @log_results()
    def _tune_parameters(self, X: np.ndarray, y: np.ndarray) -> Tuple[Dict[str, Any], float]:
        """
        ...
        :return: Best parameters and score.
        """
        grid_search = GridSearchCV(estimator=self.model_loader, param_grid=self.param_grid, cv=self.tscv,
